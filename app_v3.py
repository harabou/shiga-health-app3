...
def simulate_improvement(df, target_col, mode, rate):
    ...
    if not df_new.empty:
        ...
        # 元のデータ（人数が減ったもの）と、新しいカテゴリ（人数が増えたもの）を結合
...
with st.sidebar:
    st.header("📂 データ読み込み")
    data_file = st.file_uploader("1. data.csv (対象者データ)", type="csv")
    list_file = st.file_uploader("2. list.csv (マスタデータ)", type="csv")
    st.divider()
    st.header("⚙️ シミュレーション設定")
    f_map = {"BP_c":"血圧", "SM":"喫煙", "DM":"血糖", "BMI_c":"肥満"}
    f_label = st.selectbox("改善する項目", list(f_map.values()))
    factor = [k for k, v in f_map.items() if v == f_label][0]
    mode = st.selectbox("改善対象グループ", ["S1：Worst改善", "S2：Except Best改善"])
...
if data_file and list_file:
    try:
        ...
        if not merged_df.empty:
            ...
            # --- 【重要】改善後のカテゴリに基づき、マスタからyearを再マッピング ---
            ...
            with t2:
                ...
                df_diff = pd.merge(
                    ...
                    df_scn.groupby("category")["count"].sum().reset_index().rename(columns={"count":"scn"}), 
                ...
                if not plot_diff.empty:
                    fig_bar = px.bar(plot_diff, x="category", y="diff", color="diff", 
                                     color_continuous_scale="RdBu", title="カテゴリ別の人数増減（移動した人数）")
                    st.plotly_chart(fig_bar, use_container_width=True)
                else:
                    st.info("改善率を上げると、カテゴリ間の移動が表示されます。")

            with t3:
                # ツリーマップ
                c1, c2 = st.columns(2)
                c1.plotly_chart(px.treemap(merged_df, path=[px.Constant("現状"), "sex", "category"], 
                                           values="count", color="year", color_continuous_scale="RdBu"), use_container_width=True)
                c2.plotly_chart(px.treemap(df_scn, path=[px.Constant("改善後"), "sex", "category"], 
                                           values="count", color="year", color_continuous_scale="RdBu"), use_container_width=True)
        else:
